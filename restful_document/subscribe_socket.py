...
class SubscribeSocket(object):
    ...
    @classmethod
    def subscribe(cls):
        enforce(cls._instance is not None, _('Subscription is disabled'))
        ticket = os.urandom(16).encode('hex')
        # pylint: disable-msg=W0212
        cls._instance._tickets.add(ticket)
        return {'host': cls._instance._host,
                'port': cls._instance._port,
                'ticket': ticket,
                }
    ...
    def serve_forever(self):
        ...
        try:
            SubscribeSocket._instance = self
            self._server.serve_forever()
        finally:
            self._server.stop()
            self._server = None
            SubscribeSocket._instance = None
    ...
    def __signal_cb(self, event):
        if rd.only_sync_notification.value:
